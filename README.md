...
Three columns, one variable each, and the model never changes down a row.

**A 3B went from 7/34 to 30/34 without being swapped for anything.** It ends up
one task behind a 7B and three behind a 14B. Read as a sizing guide, the shipped
setup made a 14B look necessary; fixed, a 3B does the job.
...
Two caveats, kept in view: one model family, and 34 tasks means a single task is
three points. This is enough to falsify a wrong claim, which is what it was built
